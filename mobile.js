(() => {
  ...
  collapseButton.type = 'button'; collapseButton.className = 'collapse-toggle';
  sheet.append(collapseButton);
  function setCollapsed(collapsed){ sheet.classList.toggle('collapsed', collapsed); sheet.classList.remove('expanded'); collapseButton.textContent = collapsed ? '展开列表' : '收起列表'; collapseButton.setAttribute('aria-expanded', String(!collapsed)); }
  collapseButton.onclick = () => setCollapsed(!sheet.classList.contains('collapsed'));
  ...
  const normalize = item => ({ ...item, label: item.name || item.zh || item.en, sublabel: config.sublabel(item), kind: config.kind(item), wiki: wikiUrl(item) });
